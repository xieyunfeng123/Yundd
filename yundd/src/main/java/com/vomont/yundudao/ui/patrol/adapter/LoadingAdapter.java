...
public class LoadingAdapter extends BaseAdapter
{
    ...
    public View getView(final int position, View convertView, ViewGroup parent)
    {
        ...
        asyncloadImage(holder.load_img, mlist.get(position).getVideoPath() + "/" + mlist.get(position).getName() + ".mp4", position, mlist.get(position).getName());
    ...
    private void asyncloadImage(final ImageView imageView, final String path, final int position, final String name)
    {
        final Handler mHandler = new Handler()
        {
            ...
            public void handleMessage(Message msg)
            {
                ...
                {
                    ImageLoader.getInstance().displayImage("file://" + VideoManager.video_face_img + "/" + mlist.get(position).getName() + ".jpg", imageView);
                    // Glide.with(context.getApplicationContext()).load(new File(VideoManager.video_face_img + "/" +
                    // mlist.get(position).getName() + ".jpg")).into(imageView);
                    // .getInstance().displayFromSDCard(VideoManager.video_face_img + "/" +
                    // mlist.get(position).getName() + ".jpg", imageView);
                }
            }
        };
