...
//        Glide.with(this).load(new File(mMedias.get(nowPosition).getPath())).diskCacheStrategy(DiskCacheStrategy.NONE).skipMemoryCache(true).into( mlist.get(nowPosition));
        ...
        preview_pager.setOnPageChangeListener(new OnPageChangeListener() {
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                ...
                preview_now_num.setText((position + 1) + "");
                if (mMedias.get(position).getIsChecked())
                {
                    preview_check.setSelected(true);
                }
                else
                ...
                ImageLoader.getInstance().displayImage("file://"+mMedias.get(position).getPath(),  mlist.get(position));
            }
    ...
    private void showsendNum()
    {
        ...
        {
//            preview_ok.setTextColor(getResources().getColor(R.color.text_color));
            preview_ok.setText("确定");
        }
        else
        {
//            preview_ok.setTextColor(getResources().getColor(R.color.main_color));
