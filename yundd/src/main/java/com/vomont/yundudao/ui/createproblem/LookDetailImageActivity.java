...
public class LookDetailImageActivity extends Activity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        intent = getIntent();
        mlistPath = (List<String>)intent.getSerializableExtra("mlistPath");
        ...
        detail_img_pager.setOnPageChangeListener(new OnPageChangeListener() {
            ...
            public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                ...
                img_show_num.setText(nowPosition + 1 + "");
//                Glide.with(LookDetailImageActivity.this).load(new File(VideoManager.detail_img_cash + "/" + mlistPath.get(nowPosition) + ".jpg")).into(img_list.get(nowPosition));
                showImg();
            }
    ...
    public void showImg()
    {
        for (int i = 0; i < mlistPath.size(); i++)
        {
            if (nowPosition == Integer.parseInt(mlistPath.get(i).substring(14, 15)))
            {
                Glide.with(LookDetailImageActivity.this).load(new File(VideoManager.detail_img_cash + "/" + mlistPath.get(i) + ".jpg")).into(img_list.get(nowPosition));
            }
        }
