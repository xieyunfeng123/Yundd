...
@SuppressLint({"SimpleDateFormat", "ClickableViewAccessibility", "HandlerLeak"}) public class VideoFaceActivity extends Activity implements OnClickListener
{
...
//        }, 200);
    ...
    private void moveViewWithFinger(View view, float rawX)
    {
        if ((int)rawX >= view.getWidth())
        {
            ...
            Log.e("insert","seekto=="+videoLength * ((int)rawX - view.getWidth()) / (getWindowManager().getDefaultDisplay().getWidth() - view.getWidth()));
            videoView_faceimg.pause();
        }
